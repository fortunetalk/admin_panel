...
const TermsAndConditions = ({ dispatch, termsAndConditionData, isLoading }) => {
  ...
  useEffect(function () {
    dispatch(Actions.getAllTermCondtionList());
  }, [dispatch]);
  ...
  const handleSubmit = async () => {
    if (validation()) {
      ...
      dispatch(
        Actions.updateTermCondtion( formData)
      );
      if (isLoading) {
        setOpen(true);
      } else{
  ...
  const handleClickOpen = (rowData) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        dispatch(Actions.updateTermConditionStatus({ termId: rowData._id, status: newStatus }));
      }
    });
  };
              ...
              columns={[
                {
                  title: "S.No",
                  editable: "never",
                  render: rowData => Array.isArray(termsAndConditionData) ? termsAndConditionData.indexOf(rowData) + 1 : 'N/A'
                },
