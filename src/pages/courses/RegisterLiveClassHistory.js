...
const RegisterLiveClassHistory = ({ dispatch, registerLiveClassHistoryData }) => {
  ...
  useEffect(function () {
    dispatch(HistoryActions.getRegisterLiveClassHistory());
  }, []);
  ...
  return (
    <div className={classes.container}>
      ...
      <div className={classes.box}>
        {registerLiveClassHistoryData && displayTable()}
        {editModal()}
      </div>
    </div>
  );
    ...
    return (
      <Grid container spacing={1}>
        <Grid item lg={12} sm={12} md={12} xs={12}>
          <MaterialTable
            title=" Chat History"
            data={registerLiveClassHistoryData}
            columns={[
              {
                title: "S.No",
                editable: "never",
                render: (rowData) => registerLiveClassHistoryData.indexOf(rowData) + 1,
              },
...
const mapStateToProps = (state) => ({
  registerLiveClassHistoryData: state.history.registerLiveClassHistoryData,
});

const mapDispatchToProps = (dispatch) => ({ dispatch });

export default connect(mapStateToProps, mapDispatchToProps)(RegisterLiveClassHistory);
