...
const DisplayEcommerceBanner = ({ dispatch, ecommerceBannerData }) => {
  ...
  useEffect(function () {
    dispatch(Actions.getEcommerceBannerData());
  }, []);
  ...
  const handleSubmit = async () => {
    if (validation()) {
      ...
      dispatch(
        Actions.updateEcommerceBanner(formData)
      );
    setOpen(false);
  ...
  const handleClickOpen = (rowData) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        dispatch(Actions.updateEcommerceBannerStatus({ bannerId: rowData._id, status: newStatus }));
      }
    });
  };
      ...
      <Grid container spacing={1}>
        <Grid item lg={12} sm={12} md={12} xs={12}>
          <MaterialTable
            ...
            data={ecommerceBannerData}
            columns={[
              {
                title: "S.No",
                editable: "never",
                render: rowData => Array.isArray(ecommerceBannerData) ? ecommerceBannerData.indexOf(rowData) + 1 : 'N/A'
              },
              { title: "Ecommerce Banner", field: "title" },
...
export default connect(mapStateToProps, mapDispatchToProps)(DisplayEcommerceBanner);
