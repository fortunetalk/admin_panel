...
const DisplayCallChatBanner = ({ dispatch, callChatBannerData }) => {
  ...
  useEffect(function () {
    dispatch(Actions.getCallChatBannerData());
  }, []);
  ...
  const handleSubmit = async () => {
    if (validation()) {
      ...
      dispatch(
        Actions.updateCallChatBanner(formData)
      );
    setOpen(false);
  ...
  const handleClickOpen = (rowData) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        dispatch(Actions.updateCallChatBannerStatus({ bannerId: rowData._id, status: newStatus }));
      }
    });
  };
  ...
  return (
    <div className={classes.container}>
      <div className={classes.box}>
        {callChatBannerData && displayTable()}
        {editModal()}
      </div>
    </div>
  );
    ...
    return (
      <Grid container spacing={1}>
        <Grid item lg={12} sm={12} md={12} xs={12}>
          <MaterialTable
            title="Call-Chat"
            data={callChatBannerData}
            columns={[
              {
                title: "S.No",
                editable: "never",
                render: rowData => Array.isArray(callChatBannerData) ? callChatBannerData.indexOf(rowData) + 1 : 'N/A'
              },
...
export default connect(mapStateToProps, mapDispatchToProps)(DisplayCallChatBanner);
