...
import { setPoojaCategory } from '../../redux/Actions/poojaCategoryActions.js'
import { Colors } from "../../assets/styles.js";
import { useNavigate } from "react-router-dom";
import { useDispatch,connect } from "react-redux";

export const CreatePoojaCategory = () => {
  ...
  const handleSubmit = async () => {
    if (validation()) {
      ...
      dispatch(setPoojaCategory(formData));
      // navigate('/fortune-store/pooja-category')
    }
  };
          ...
          <Grid container spacing={2}>
            <Grid item lg={12} sm={12} md={12} xs={12}>
              <div className={classes.headingContainer}>
                <div className={classes.heading}>Add Pooja Category</div>
                <div
                  onClick={() => navigate("/fortune-store/pooja-category")}
                  className={classes.addButton}
                >
                  <DvrIcon />
                  <div className={classes.addButtontext}>
                    Display/Create Pooja Category
                  </div>
                </div>
              </div>
            </Grid>
...
export default connect(mapStateToProps, mapDispatchToProps)(CreatePoojaCategory);
