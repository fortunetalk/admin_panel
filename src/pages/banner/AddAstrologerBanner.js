...
const AddAstrologerBanner = ({ dispatch, isLoading }) => {
  ...
  const handleSubmit = async () => {
    if (validation()) {
      ...
      dispatch(Actions.createAstrologerBanner(formData));
      handleReset();
    }
  };
        ...
        <Grid container spacing={2}>
          <Grid item lg={12} sm={12} md={12} xs={12}>
            <div className={classes.headingContainer}>
              <div className={classes.heading}>Add Astrologer Banner</div>
              <div
                onClick={() => navigate("/displayAstrologerBanner")}
                className={classes.addButton}
              >
                <DvrIcon />
                <div className={classes.addButtontext}>Display Astrologer Banner</div>
              </div>
            </div>
          </Grid>
...
export default connect(mapStateToProps, mapDispatchToProps)(AddAstrologerBanner);
